...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose, onCheckoutComplete }) => {
	...
	}, [depositReturnPrice, depositReturnCount, cart]);
						...
						<div className={styles.gridRow}>
							...
							>
								<div className={styles.priceInput}>
									<input type="number" step="0.01" min="0" value={depositReturnPrice} onChange={(e) => setDepositReturnPrice(e.target.value)} placeholder="2,00" className={styles.priceField} />
									<span>€</span>
								</div>
								...
								<div className={styles.countingControls}>
									<button onClick={() => setDepositReturnCount(Math.max(0, depositReturnCount - 1))} className={styles.countingBtn} disabled={depositReturnCount <= 0}>
										<MdIcons.MdRemove size={14} />
									</button>

									<span className={styles.countingDisplay}>{depositReturnCount}</span>

									<button onClick={() => setDepositReturnCount(depositReturnCount + 1)} className={styles.countingBtn}>
										<MdIcons.MdAdd size={14} />
									</button>
								</div>
