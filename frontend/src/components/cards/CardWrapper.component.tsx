		...
		<div className={styles.cardWrapper}>
			...
			{isEditing ? (
				<div className={styles.iconBtnGroup}>
					{onSaveAction && item && (
						<button type="button" title="Speichern" className={styles.iconBtn} onClick={() => onSaveAction(item)}>
							<MdSave size={20} />
						</button>
					)}
					{onRequestView && (
						<button type="button" title="Abbrechen" className={styles.iconBtn} onClick={onRequestView}>
							<MdClose size={20} />
						</button>
					)}
				</div>
				...
				<div className={styles.iconBtnGroup}>
					{onRequestEdit && !editLocked && (
						<button type="button" title="Bearbeiten" className={styles.iconBtn + ' .icon'} onClick={onRequestEdit}>
							<MdEdit size={20} />
						</button>
					)}
			...
			{/* Delete Confirmation Modal replaced with native dialog */}
		</div>
	);
}
