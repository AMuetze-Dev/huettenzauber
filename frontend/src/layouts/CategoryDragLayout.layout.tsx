...
export default function CategoryDragLayout({
  categories,
  items,
  selectedCategoryId,
  onCategorySelect,
  ...
  const handleItemDrop = (
    ...
    const categoryItems = getItemsByCategory(targetCategoryId);
    const draggedFromSameCategory =
      draggedItem.category_id === targetCategoryId;

    if (draggedFromSameCategory) {
      ...
      onItemReorder(targetCategoryId, reorderedItems);
    } else {
      // Move to different category
      const updatedItem = { ...draggedItem, category_id: targetCategoryId };
      const newCategoryItems = [...categoryItems];
      newCategoryItems.splice(targetIndex, 0, updatedItem);

      onItemReorder(targetCategoryId, newCategoryItems);
    }
  };
    ...
    <div className={styles.categoryDragLayout}>
      ...
      <div className={styles.categoryHeaders}>
        {categories.map((category) => {
          const IconComponent =
            MdIcons[category.icon as keyof typeof MdIcons] ||
            MdIcons.MdCategory;
