...
const ServiceContent: React.FC = () => {
	...
	const toggleLabel = collapsed ? TEXT_LABELS.SIDEBAR_EXPAND : TEXT_LABELS.SIDEBAR_MINIMIZE;
	const themeIcon = theme === 'dark' ? THEME_ICONS.LIGHT_MODE : THEME_ICONS.DARK_MODE;
	const themeLabel = theme === 'dark' ? TEXT_LABELS.LIGHT_MODE : TEXT_LABELS.DARK_MODE;
	const themeTitle = theme === 'dark' ? TEXT_LABELS.LIGHT_MODE_ACTIVATE : TEXT_LABELS.DARK_MODE_ACTIVATE;
	...
	const renderCategoryItem = (category: any) => {
		...
		const isActive = activeSection === 'category' && selectedCategoryId === category.id;
		...
		return (
			<li key={category.id} data-priority={getCategoryPriority(category.name)}>
				<button type="button" className={isActive ? 'active' : ''} onClick={() => handleCategorySelect(category.id)} onTouchStart={showTouchLabels ? () => handleIconTouch(`category-${category.id}`) : undefined} title={`${category.name} - Artikel anzeigen`}>
					<IconComponent size={UI_CONFIG.ICON_SIZE} />

					{/* Desktop: Normal label when expanded */}
					{!collapsed && !isTouchDevice && <span className={style['nav-label']}>{category.name}</span>}

					{/* Touch: Overlay label when touched */}
					{showTouchLabels && hoveredItem === `category-${category.id}` && <span className={style['touch-label']}>{category.name}</span>}
				</button>
			</li>
		);
	};
	...
	const renderNavigationItem = (item: NavigationItem, isBottomNav = false) => {
		...
		return (
			<li key={item.id}>
				<button type="button" className={isActive ? 'active' : ''} onClick={isBottomNav ? () => handleSectionChange(item.id) : undefined} onTouchStart={showTouchLabels ? () => handleIconTouch(`nav-${item.id}`) : undefined} title={item.description}>
		...
		<div className={style.layout}>
			<aside className={`${style.navbar} ${collapsed ? style.collapsed : ''}`}>
				...
				<div>
					...
					{showToggleButton && (
						<button type="button" onClick={handleToggleSidebar} title={toggleLabel} className={style['toggle-button']}>
							{React.createElement(toggleIcon, { size: UI_CONFIG.ICON_SIZE })}
							{!collapsed && <span className={style['nav-label']}>{TEXT_LABELS.NAVIGATION}</span>}
						</button>
					)}

					{/* Categories Navigation */}
					<ul className={style['nav-top']}>{categories.map((category) => renderCategoryItem(category))}</ul>
				</div>
				...
				<div>
					...
					<div className={style['nav-theme']}>
						<button type="button" onClick={switchTheme} onTouchStart={showTouchLabels ? () => handleIconTouch('theme-toggle') : undefined} title={themeTitle}>
							{React.createElement(themeIcon, { size: UI_CONFIG.ICON_SIZE })}

							{/* Desktop: Normal label when expanded */}
							{!collapsed && !isTouchDevice && <span className={style['nav-label']}>{themeLabel}</span>}

							{/* Touch: Overlay label when touched */}
							{showTouchLabels && hoveredItem === 'theme-toggle' && <span className={style['touch-label']}>{themeLabel}</span>}
						</button>
					</div>

					{/* Bottom Navigation Items */}
					<ul className={style['nav-bottom']}>{BOTTOM_NAVIGATION_CONFIG.map((item) => renderNavigationItem(item, true))}</ul>
				</div>
			</aside>
