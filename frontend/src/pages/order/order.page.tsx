...
const VariantItem: React.FC<VariantItemProps> = ({ item, variant, currentQuantity, onAdd, onRemove, formatPrice }) => {
	...
	return (
		<div className={`${styles.variantCompact} ${currentQuantity > 0 ? styles.inCart : ''}`} onMouseDown={handleStart} onTouchStart={handleStart} onMouseUp={handleEnd} onTouchEnd={handleEnd} onMouseLeave={handleLeave}>
			...
			<div className={styles.touchHint}>{currentQuantity === 0 ? <span>Tippen zum Hinzufügen</span> : <span>Tippen: +1 | Lang: -1</span>}</div>
		</div>
	);
};
		...
		<div className={styles.container}>
			...
			<main className={styles.main}>
				...
				{filteredItems.length === 0 ? (
					<div className={styles.emptyState}>
						...
						<p>{selectedCategoryId ? 'In dieser Kategorie sind keine Artikel verfügbar.' : 'Wähle eine Kategorie aus der Sidebar.'}</p>
					</div>
				) : (
							...
							<div key={item.id} className={styles.itemCard}>
								...
								<div className={styles.variants}>
									{item.item_variants.map((variant: ItemVariant) => {
										const currentQuantity = cart.getItemQuantity(item.id, variant.id);

										return <VariantItem key={variant.id} item={item} variant={variant} currentQuantity={currentQuantity} onAdd={() => handleVariantAdd(item, variant)} onRemove={() => handleVariantRemove(item, variant)} formatPrice={formatPrice} />;
									})}
								</div>
							</div>
						))}
			...
			{cart.state.items.length > 0 && selectedCategoryId && (
				<div className={styles.cartFloating}>
					<button className={styles.cartButton} onClick={() => setIsCartModalOpen(true)} title="Warenkorb anzeigen und bearbeiten">
						<div className={styles.cartIcon}>
							<MdIcons.MdShoppingCart size={24} />
							<span className={styles.cartBadge}>{cart.state.totalItems}</span>
						</div>
						<div className={styles.cartInfo}>
							<span className={styles.cartItemCount}>{cart.state.totalItems} Artikel</span>
							<span className={styles.cartTotal}>{formatPrice(cart.state.totalAmount + cart.state.totalDepositAmount)}</span>
						</div>
					</button>
				</div>
			)}
			...
			{/* Cart Modal */}
			<CartModal isOpen={isCartModalOpen} onClose={() => setIsCartModalOpen(false)} onCheckoutComplete={handleCheckout} />
		</div>
	);
};
...
const OrderPage: React.FC<OrderPageProps> = ({ selectedCategoryId, onCategoryChange }) => {
	return (
		<ProductProvider>
			<OrderContent selectedCategoryId={selectedCategoryId} onCategoryChange={onCategoryChange} />
		</ProductProvider>
	);
};
