...
const calculateTotals = (items: CartItem[]) => {
	...
	return {
		...
		totalDepositAmount: visibleItems.reduce((sum, item) => sum + (item.depositAmount || 0) * item.quantity, 0),
	};
};
...
const cartReducer = (state: CartState, action: CartAction): CartState => {
	...
	switch (action.type) {
		...
		case 'UPDATE_QUANTITY': {
			...
			const newItems = state.items.map((item) => (item.id === action.payload.id ? { ...item, quantity: action.payload.quantity } : item));
		...
		case 'INCREASE_QUANTITY': {
			const newItems = state.items.map((item) => (item.id === action.payload.id ? { ...item, quantity: item.quantity + 1 } : item));
		...
		case 'SET_DEPOSIT_RETURN': {
			...
			// Spezialbehandlung für 0-Werte (Zurücksetzen)
			if (action.payload.pricePerItem === 0 || action.payload.quantity === 0) {
				// Prüfe ob bereits undefined oder 0-Werte
				if (!state.depositReturn || state.depositReturn.totalReturn === 0) {
					return state; // Keine Änderung nötig
				}

				newState = {
					...state,
					depositReturn: undefined, // Setze auf undefined statt 0-Objekt
				};
				break;
			}
			...
			if (currentDepositReturn && currentDepositReturn.pricePerItem === action.payload.pricePerItem && currentDepositReturn.quantity === action.payload.quantity && currentDepositReturn.totalReturn === totalReturn) {
...
export const PersistentCartProvider: React.FC<CartProviderProps> = ({ children }) => {
	...
	const setDepositReturn = useCallback((pricePerItem: number, quantity: number) => {
		dispatch({
			type: 'SET_DEPOSIT_RETURN',
