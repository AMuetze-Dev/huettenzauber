...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, onConfirm }) => {
	...
	const calculateChange = (): number => {
		...
		const totalWithDeposit = cart.state.totalAmount + cart.state.totalDepositAmount;
		return received + depositReturn - totalWithDeposit;
	};

	const getTotalToPay = (): number => {
		const depositReturn = parseFloat(depositReturnAmount) || 0;
		return cart.state.totalAmount + cart.state.totalDepositAmount - depositReturn;
	};
			...
			<div className={styles.modal} onClick={handleModalClick}>
				...
				<div className={styles.content}>
					...
					<div className={styles.orderSummary}>
						...
						<div className={styles.totalSection}>
							<div className={styles.totalRow}>
								...
								<span className={styles.totalAmount}>{formatPrice(cart.state.totalAmount)}</span>
							</div>
							{cart.state.totalDepositAmount > 0 && (
								<div className={styles.totalRow}>
									<span className={styles.totalLabel}>Pfand:</span>
									<span className={styles.totalAmount}>{formatPrice(cart.state.totalDepositAmount)}</span>
								</div>
							)}
							<div className={styles.totalRow + ' ' + styles.grandTotal}>
								<span className={styles.totalLabel}>Gesamtbetrag:</span>
								<span className={styles.totalAmount}>{formatPrice(cart.state.totalAmount + cart.state.totalDepositAmount)}</span>
							</div>
							{showDepositReturn && parseFloat(depositReturnAmount) > 0 && (
								<>
									<div className={styles.totalRow}>
										<span className={styles.totalLabel}>Pfandrückgabe:</span>
										<span className={styles.totalAmount} style={{ color: 'var(--color-success, #10b981)' }}>
											- {formatPrice(parseFloat(depositReturnAmount))}
										</span>
									</div>
									<div className={styles.totalRow + ' ' + styles.grandTotal}>
										<span className={styles.totalLabel}>Zu zahlen:</span>
										<span className={styles.totalAmount}>{formatPrice(getTotalToPay())}</span>
									</div>
								</>
							)}
						</div>
					...
					{/*<div className={styles.changeCalculator}>
						<div className={styles.calculatorHeader}>
							<label className={styles.checkboxLabel}>
								<input type="checkbox" checked={showChangeCalculator} onChange={(e) => setShowChangeCalculator(e.target.checked)} />
								Rückgeldrechner
							</label>
						</div>
							...
							<div className={styles.calculatorContent}>
								<div className={styles.inputGroup}>
									...
									<div className={styles.amountInput}>
										<input id="receivedAmount" type="number" step="0.01" min="0" value={receivedAmount} onChange={(e) => setReceivedAmount(e.target.value)} placeholder="0,00" />
					...
					<div className={styles.changeCalculator}>
						<div className={styles.calculatorHeader}>
							<label className={styles.checkboxLabel}>
								<input type="checkbox" checked={showDepositReturn} onChange={(e) => setShowDepositReturn(e.target.checked)} />
								Pfandrückgabe
							</label>
						</div>
						...
						{showDepositReturn && (
							<div className={styles.calculatorContent}>
								<div className={styles.inputGroup}>
									<label htmlFor="depositReturnAmount">Pfandrückgabe-Betrag:</label>
									<div className={styles.amountInput}>
										<input id="depositReturnAmount" type="number" step="0.01" min="0" value={depositReturnAmount} onChange={(e) => setDepositReturnAmount(e.target.value)} placeholder="0,00" />
										<span className={styles.currency}>€</span>
									</div>
								</div>
								...
								{depositReturnAmount && parseFloat(depositReturnAmount) > 0 && (
									<div className={styles.changeResult}>
										<div className={styles.changeInfo}>
											...
											<span>{formatPrice(cart.state.totalAmount + cart.state.totalDepositAmount)}</span>
										</div>
										<div className={styles.changeInfo}>
											<span>Pfandrückgabe:</span>
											<span style={{ color: 'var(--color-success, #10b981)' }}>- {formatPrice(parseFloat(depositReturnAmount))}</span>
										</div>
										<div className={styles.changeAmount}>
											<span>Zu zahlen:</span>
											<span className={styles.positive}>{formatPrice(getTotalToPay())}</span>
										</div>
									</div>
								)}
							</div>
				...
				<div className={styles.actions}>
					...
					<button className={styles.confirmButton} onClick={handleConfirmOrder} disabled={isProcessing || (showChangeCalculator && calculateChange() < 0)}>
						{isProcessing ? (
							<>
