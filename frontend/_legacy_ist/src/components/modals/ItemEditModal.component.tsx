...
export default function ItemEditModal({ isOpen, item, isNewItem = false, onSave, onClose }: ItemEditModalProps) {
	...
	const removeVariant = (index: number) => {
		const updatedVariants = editedItem.item_variants.filter((_, i) => i !== index);
		setEditedItem({ ...editedItem, item_variants: updatedVariants });
	};
	...
	const isSimpleItem = editedItem.item_variants.length === 1 && !editedItem.item_variants[0].name;
				...
				>
					...
					{isSimpleItem ? (
							...
							>
								...
								>
									...
									<input
										...
										placeholder="0.00"
										onChange={(e) => handleSimplePriceChange(parseFloat(e.target.value) || 0)}
										style={{
											padding: 'var(--spacing-md)',
											border: '1px solid var(--card-border)',
											borderRadius: 'var(--radius-md)',
								...
								>
									...
									<input
										...
										placeholder="1"
										onChange={(e) => handleSimpleStepsChange(e.target.value === '' ? 0 : parseFloat(e.target.value))}
										style={{
											padding: 'var(--spacing-md)',
											border: '1px solid var(--card-border)',
											borderRadius: 'var(--radius-md)',
						...
						>
							...
							>
								<h4 style={{ margin: 0, color: 'var(--text-primary)' }}>Varianten</h4>
								<div style={{ display: 'flex', gap: 'var(--spacing-md)' }}>
									...
									{(editedItem.item_variants.length > 1 || (editedItem.item_variants.length === 1 && editedItem.item_variants[0].name)) && (
										<button
											type="button"
									...
									>
										...
										<div
											style={{
												...
												flexDirection: 'column',
												gap: 'var(--spacing-md)',
												marginRight: editedItem.item_variants.length > 1 ? '40px' : '0',
											}}
										>
											<div
												style={{
											...
											>
												...
												>
													...
													<input
														...
														placeholder="1"
														onChange={(e) => handleVariantChange(index, 'bill_steps', e.target.value === '' ? 0 : parseFloat(e.target.value))}
														style={{
															padding: 'var(--spacing-md)',
															border: '1px solid var(--card-border)',
															borderRadius: 'var(--radius-md)',
