...
export default function CategoryCardFeature() {
	...
	const handleModalSave = useCallback(
		async (updatedCategory: Category) => {
			...
			if (isNewCategory) {
				// Optimistic Add
				setLocalCategories((prev) => [...prev, { ...updatedCategory, id: Math.max(0, ...prev.map((c) => c.id)) + 1 }]);
				await ctx.addCategory(updatedCategory);
			} else {
				// Optimistic Edit
				setLocalCategories((prev) => prev.map((c) => (c.id === updatedCategory.id ? updatedCategory : c)));
	...
	const handleCategoryReorder = useCallback(
		async (newOrder: number[]) => {
			...
			// Optimistic reorder
			const reorderedCategories = newOrder.map((id) => localCategories.find((cat) => cat.id === id)).filter(Boolean) as Category[];
	...
	const cards = [
		...
		...localCategories.map((category) => {
			const hasStockItems = ctx.items.some((item) => item.category_id === category.id);
			return (
				<CardWrapper
					key={category.id}
					isEditing={false}
					...
					onDeleteAction={hasStockItems ? undefined : handleDelete}
					item={category}
					itemId={category.id}
					deleteDisabled={hasStockItems}
				>
					{() => <CategoryCard id={category.id} name={category.name} icon={category.icon} onDragStart={handleDragStart} onDragEnd={handleDragEnd} isDragging={draggedCategory?.id === category.id} />}
				</CardWrapper>
			);
		}),
		// Add card
		<CardWrapper key="add-card" isEditing={false} editLocked={false} onRequestEdit={undefined} onRequestView={undefined} onSaveAction={undefined} onDeleteAction={undefined} item={null} itemId={-999}>
	...
	return (
		<>
			...
			{/* Edit Modal */}
			<CategoryEditModal isOpen={isModalOpen} category={editingCategory} isNewCategory={isNewCategory} onSave={handleModalSave} onClose={handleModalClose} />
		</>
	);
}
